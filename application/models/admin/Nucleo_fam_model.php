...
class Nucleo_fam_model extends CI_Model
{
    public $v_fields=array('nucleo_fam');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->order_by("id_nucleofam","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('id_nucleofam' => $id));
        $data = $query->result();
        return $data[0];
    }
