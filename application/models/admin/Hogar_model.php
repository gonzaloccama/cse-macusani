...
{
    public $v_fields=array('no_ficha', 'direccion', 'barrio.barrio', 'ciudad.ciudad', 'fecha_registro', 'cse.cse');
	...
	function getList($table, $pagination=array()) {
        ...
        $this->db->select("$table.*  , barrio.barrio as barrio  , ciudad.ciudad as ciudad  , cse.cse as cse ");
        $this->db->from($table);
         $this->db->join("barrio", "hogar.barrio = barrio.id_barrio", "left");  $this->db->join("ciudad", "hogar.ciudad = ciudad.id_ciudad", "left");  $this->db->join("cse", "hogar.cse = cse.id_cse", "left");     
        $this->db->order_by("no_ficha","desc");
        $query = $this->db->get();
    ...
    function getRow($table, $id) {
        $this->db->select('*');
        $query = $this->db->get_where($table, array('no_ficha' => $id));
        $data = $query->result();
        return $data[0];
    }
