...
class personalist_model extends CI_Model

{

	public $v_fields = array('dni', 'nombres', 'ape_pat', 'ape_mat', 'fech_naci', 'sexo.sexo', 'nucleo_fam.nucleo_fam', 'celular', 'observations', 'fecha_registro', 'hogar.no_ficha');
	...
	function getList($table, $pagination = array())
	{
		...
		$this->db->select("$table.*  , sexo.sexo as sexo  , nucleo_fam.nucleo_fam as nucleo_fam  , hogar.no_ficha as no_ficha ");

		$this->db->from($table);

		$this->db->join("sexo", "persona.sexo = sexo.id_sexo", "left");
		$this->db->join("nucleo_fam", "persona.nucleo_fam = nucleo_fam.id_nucleofam", "left");
		$this->db->join("hogar", "persona.no_ficha = hogar.no_ficha", "left");
